...
| matches/match_participants 보호 트리거 | ✅   | `supabase/migrations/20260516_tighten_matches_participant_update.sql` + `..._match_participants_self_update.sql` | `participant_update` WITH CHECK + `prevent_protected_matches_update` 트리거(7개 컬럼 잠금) / `self_update` 정책 + `prevent_protected_match_participants_update` 트리거(6개 컬럼 잠금, score만 인가 사용자 갱신). service_role은 `auth.role()='service_role'` 우회 (PR #16) |
...
- `match_participants` RLS: `match_participants_co_participant_read` (TO authenticated, 자기 row OR 같은 매치 참가자 row — anon 차단, PR #14) / `self_insert` / `self_update` (TO authenticated, user_id = auth.uid(), USING + WITH CHECK — PR #16 회귀 fix) / `self_delete`
- **BEFORE UPDATE 트리거** (PR #16): `prevent_protected_matches_update` (matches 7개 컬럼 OLD 고정) / `prevent_protected_match_participants_update` (match_participants 6개 컬럼 OLD 고정, score만 인가 사용자 갱신 허용). 양쪽 모두 `auth.role() = 'service_role'` 분기로 service-role 호출 우회
