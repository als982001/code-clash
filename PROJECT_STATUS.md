...
- **B-12 백업 테이블 `problems_description_backup_20260606` DROP** — Supabase 보안 advisor `rls_enabled_no_policy`(정책 없는 RLS = 접근 불가라 현재 위험 낮음). 9 rows 보유, 앱 코드 미참조. **비가역(데이터 영구 삭제)이라 별도 명시 승인 필요** — DROP 전 CSV/dump 백업 권장. (PR #37 advisor 점검 잔여 NEEDS-CARE 항목)
- **(advisor 무시 항목) `auth_leaked_password_protection` 비활성** — OAuth(Google/GitHub) 전용·비밀번호 미사용이라 무관. 조치 불필요.
...
- `public.get_match_history(p_user_id uuid, p_limit integer DEFAULT 20)` — SECURITY DEFINER STABLE, **authenticated EXECUTE 단독**(anon 명시 REVOKE, `aclexplode` 검증: postgres/service_role/authenticated만). 특정 유저 관점의 finished 매치 리스트 반환: `match_id`/`result`(win·loss·draw, winner_id 기준)/`problem_title`/`opponent_id`/`opponent_nickname`/`opponent_avatar_url`/`my_mmr_change`/`end_time`/`created_at`. 상대 조인 `LEFT JOIN LATERAL (... LIMIT 1)`로 매치당 정확히 1행 보장(미래 다인 매치 도입 시 matchId 행 중복 방지). 반환 PII는 공개 프로필(닉네임/아바타)뿐 — 비밀 토큰 제외. 대전 히스토리 표시용 (2026-06-13, Post-MVP A-5)
- `public.cleanup_stale_matches_and_queue()` — SECURITY DEFINER, **service_role EXECUTE 단독**(anon/authenticated 명시 REVOKE). pg_cron 15분 주기 호출용 좀비 정리 함수(B-5/B-7). 4 카테고리: ① stale ongoing+제출있음→finished 몰수 종료(distinct 제출자 1명→winner, 그외 NULL, MMR 미변동) ② stale ongoing+제출없음→DELETE(match_participants CASCADE) ③ stale waiting(invite_expires_at 만료 OR 1h)→DELETE ④ matchmaking_queue 15분 초과→DELETE. 모든 매치 DELETE에 `NOT EXISTS(submissions)` 가드(FK NO ACTION 에러 차단). 반환 `(finalized, deleted_ongoing, deleted_waiting, deleted_queue)`. **DB 적용·검증 완료(사용자 직접 + `aclexplode` anon/authenticated 부재 확인, pg_cron job `cleanup-stale-matches` active)** (2026-06-14, Post-MVP B-5/B-7)
...
| `20260613_get_match_history_rpc.sql`                        | **Post-MVP A-5(대전 히스토리)** — `get_match_history(p_user_id, p_limit)` SECURITY DEFINER STABLE RPC 신설. 특정 유저 관점 finished 매치 리스트(match_id/result/problem_title/opponent_id/opponent_nickname/opponent_avatar_url/my_mmr_change/end_time) 반환. 상대 조인 `LEFT JOIN LATERAL (... LIMIT 1)`로 매치당 1행 보장. `REVOKE FROM public` + `REVOKE FROM anon, authenticated` + `GRANT TO authenticated`(PR #24 함정 회피). **DB 적용 완료(사용자 직접) + `aclexplode` 검증(postgres/service_role/authenticated만, anon 없음) + 본문 로직 직접 검증(finished 11건 win/loss/draw 정상)**. 멱등 CREATE OR REPLACE |
| `20260614_cleanup_stale_matches.sql`                        | **1회 정리(B-5/B-7 선행)** — 테스트 누적 stale 매치(ongoing/waiting 중 1일+ 경과) 22건 1회 DELETE(match_participants 35건 CASCADE, finished 보존). **DB 적용 완료(사용자 직접, stale 22→0 확인)**. 주기 함수(아래)가 포괄하므로 사실상 대체되나 무해                                                                                                                                                                                                                                                                                                                                                                    |
| `20260614_zombie_cleanup_function.sql` (1/2)                | **Post-MVP B-5/B-7(좀비 자동 정리)** — `cleanup_stale_matches_and_queue()` SECURITY DEFINER 함수 신설(4 카테고리: ① ongoing+제출있음→finished 몰수 ② ongoing+제출없음→DELETE ③ waiting 만료·1h→DELETE ④ queue 15min→DELETE, 모든 DELETE에 `NOT EXISTS(submissions)` 가드). `REVOKE FROM PUBLIC` + `REVOKE FROM anon, authenticated` + `GRANT TO service_role`. BEGIN/COMMIT 원자. 멱등 CREATE OR REPLACE. **DB 적용·검증 완료(사용자 직접, `aclexplode` postgres/service_role만)**                                                                                                                                      |
| `20260614_zombie_cleanup_schedule.sql` (2/2)                | **Post-MVP B-5/B-7** — pg_cron 등록(`CREATE EXTENSION IF NOT EXISTS pg_cron` + 동명 job DO 블록 unschedule 후 `cron.schedule('cleanup-stale-matches', '*/15 * * * *', ...)`). 비트랜잭션(CREATE EXTENSION 트랜잭션 실패 회피), 멱등. function 파일 적용 후 실행. **DB 적용·검증 완료(사용자 직접, pg_cron 설치됨 + job `cleanup-stale-matches` active=true)**                                                                                                                                                                                                                                                           |
...
- **2026-06-21** — **🛡️ 보안 보강 (Supabase advisor 대응, PR #37)**. 배포 후 보안 advisor 점검 → 안전 확정 항목만 처리(3-에이전트 워크플로우, opus). 마이그레이션 2종: ① `20260621_revoke_helper_function_exec.sql` — `handle_new_user`/`rls_auto_enable`(트리거 전용) anon·authenticated·public EXECUTE REVOKE + `get_profile_stats` anon만 REVOKE(authenticated 유지). `is_match_participant`는 RLS USING 절 사용으로 **의도적 제외**. ② `20260621_tighten_matches_insert_policy.sql` — `anon_insert(WITH CHECK true)` → `matches_host_insert(authenticated, WITH CHECK host_id=auth.uid() AND status='waiting' AND winner_id IS NULL)`. 1차(host_id만)에서 코드리뷰 P2(인증 유저가 status='finished'+winner_id=self INSERT로 가짜 전적 파밍 — get_profile_stats/get_leaderboard winner_id 집계 오염, MMR은 트리거 보호라 순위 불변) 발견 → 컬럼값(status/winner_id) 검증 추가로 차단. invite route(status='waiting'+winner_id null)는 통과·자동매칭(service-role)은 RLS 우회라 회귀 없음. **프로덕션 DB 적용·스모크(초대 매치 생성)·`pg_policies`/`aclexplode` 실측 검증 완료**. Code Reviewer(opus) Critical/Warning 0(헤더 정합성 Warning 1건 반영). 후속: 백업 테이블 DROP(아래 후속 정리 후보). Nit-2(`match_participants.self_insert` WITH CHECK)는 **이미 존재 확인 → 닫음**.
