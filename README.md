...
> 📅 **2026-05-17 기준** · Step 3 완료 (PR #18 머지 대기) — 인증(OAuth)·친구 초대 매칭·매치 보안·글로벌 헤더·프로필 페이지 모두 완료
...
- [x] **Step 3** — 인증 + 친구 초대 매칭 + 프로필 (PR #18 머지 시 종료)
  - ✅ Supabase Auth (Google / GitHub OAuth)
  - ✅ 친구 초대 링크 매칭 + 호스트 대기 화면 + 실시간 매치 상태 동기화
  - ✅ 매치 보안 강화 (RLS 좁힘 + invite token 검증 + service-role 분리)
  - ✅ (main) 글로벌 헤더 (PR #17 — `(main)/layout.tsx` + 모든 보호 라우트에 UserMenu 자동 마운트)
  - ✅ 프로필 페이지 (`/profile/[userId]`, `/profile/me`, 닉네임 편집, 자동 닉네임 fallback 모달, 누적 전적 RPC) — PR #18 머지 대기
