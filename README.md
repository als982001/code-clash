...
> 📅 **2026-06-14 기준** · MVP 핵심 완료 + Post-MVP 확장 — 인증·친구 초대 매칭·자동 매칭 큐·프로필·결과 화면·AI 코드 리뷰·MMR·리더보드까지 핵심 루프(매칭 → 대전 → 결과 → AI 리뷰 → MMR → 리더보드) 완성. 리더보드 전적·대전 히스토리·좀비 자동 정리까지 반영
...
- 각 행에 누적 전적 표시(`7승 4패 · 64%`) — 승률은 무승부 포함 분모, 0전이면 "전적 없음"
...
- 개인 프로필(`/profile/[userId]`) — 아바타·닉네임·MMR/티어 배지·가입일·한 줄 소개·누적 전적
- 본인 프로필은 닉네임/소개 인라인 편집, 자동 생성 닉네임이면 변경 권유 모달
- 대전 히스토리 — 참여한 매치 리스트(상대·승/패/무·문제·날짜·MMR 변동), 행 클릭 시 결과 화면 이동
- 본인·타인 프로필 모두 최근 20건 노출 (익명 상대는 "익명 상대"로 표기)

![프로필 & 대전 히스토리 화면](public/images/profile-overview.png)
...
- [~] **Step 5** — 커뮤니티
  ...
  - ✅ 리더보드 전적 표시 (승/패/승률 집계 RPC, `get_leaderboard`, Post-MVP A-1 후속, PR #30)
  - ✅ 대전 히스토리 (`/profile/[userId]` 매치 리스트, `get_match_history` RPC, Post-MVP A-5, PR #31)
  - [ ] 프로필 역량 분석 (태그별 방사형 차트, **Post-MVP** — 매치 데이터 축적 후)
  - [ ] AI 대전 상대 (Bot, **Post-MVP**)
- [x] **운영/인프라**
  - ✅ 좀비 매치·큐 자동 정리 (`cleanup_stale_matches_and_queue()` + pg_cron 15분 주기, B-5/B-7, PR #32)
