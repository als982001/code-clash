...
- [ ] `app/api/match/[matchId]/submit/route.ts`의 `DEFAULT_MATCH_TIME_LIMIT`를 `1800` → `900`으로 수정

**이유**: WORK_PLAN의 "15분 카운트다운"과 점수 산식의 `T_max`를 일치시켜야 시간 보너스가 정합성 있게 계산된다.

### 2-0-2. sonner 토스트 라이브러리 설치

- [ ] `npx shadcn@latest add sonner` 실행
- [ ] 자동 생성된 `components/ui/sonner.tsx` 확인
- [ ] `app/layout.tsx`의 `<body>` 내부(QueryProvider 바깥)에 `<Toaster />` 배치
...
- [ ] `app/features/match/hooks/useMatchTimer.ts` 생성
- [ ] `matches.start_time`을 기준으로 남은 시간 계산
- [ ] `setInterval(1000ms)` 대신 **`Date.now()` 기반 재계산**으로 백그라운드 탭 정확도 보장
- [ ] 반환값: `{ remainingSeconds, isWarning, isCritical, isExpired }`
...
- [ ] `app/features/match/components/MatchTimer.tsx` 생성
- [ ] `useMatchTimer` 훅 사용
- [ ] 표기 포맷: `mm:ss`
- [ ] 상태별 색상/애니메이션 적용
...
- [ ] `/play/[matchId]/page.tsx`에서 `isExpired === true`이고 내가 아직 제출하지 않았다면, 현재 에디터 코드(또는 빈 코드)로 `handleSubmit`을 자동 호출한다.
...
- [ ] `app/features/editor/components/EditorPanel.tsx` 142-157줄의 상대 프로그레스 바 블록 제거
- [ ] `EditorPanel`의 `opponentProgress` prop 제거

**이유**: Step 1에서는 에디터 하단에 작게 붙어있어 시야에 들어오기 어려웠다. 긴장감을 주려면 상단 헤더로 이동해 양쪽 바를 나란히 비교할 수 있게 한다.

### 2-2-2. 통합 헤더 컴포넌트

- [ ] `app/features/match/components/MatchStatusBar.tsx` 생성
- [ ] 레이아웃: `[타이머] [나: ▓▓▓░░ 3/5] vs [상대: ▓▓░░░ 2/5] [🔊 음소거 토글]`
- [ ] 내 바는 초록, 상대 바는 빨강
- [ ] 프로그레스 변화 시 `transition-all duration-500` 부드러운 애니메이션

**파일 경로**: `app/features/match/components/MatchStatusBar.tsx`

### 2-2-3. 내 진행률 상태 추가

- [ ] `/play/[matchId]/page.tsx`에 `myProgress` 상태 추가 (`passedCount`, `totalCount`)
- [ ] `handleRun` 내부에서 `judgeResult`를 받을 때 `setMyProgress`로 업데이트
- [ ] 이미 존재하는 `PROGRESS_UPDATE` 브로드캐스트는 그대로 유지 (상대 시점용)
...
- [ ] `/play/[matchId]/page.tsx` 315-319줄의 `opponentSubmitted` 기반 배너 제거
- [ ] `opponentSubmitted` 상태는 **제거하지 않고** 유지한다 (다른 UI 분기에서 쓰일 수 있음)

### 2-3-2. sonner 토스트 호출

- [ ] `handleOpponentSubmitted` 콜백 내부에서 `toast.warning("상대방이 최종 제출을 완료했습니다!", { duration: 5000 })` 호출
- [ ] 같은 지점에서 사운드도 트리거 (작업 2-4 참고)
...
- [ ] `app/shared/stores/useSoundStore.ts` 생성
- [ ] Zustand + `persist` 미들웨어 사용
- [ ] localStorage 키: `code-clash-sound`
...
- [ ] `app/features/match/hooks/useMatchSounds.ts` 생성
- [ ] `playSound({ type })` 함수 노출
- [ ] 타입: `"submit" | "opponentSubmit" | "warning" | "win" | "lose"`
- [ ] 내부적으로 `Audio` 객체를 미리 생성해 풀링 (재생 지연 최소화)
- [ ] `isMuted`면 no-op

### 2-4-3. 음소거 토글 버튼

- [ ] `app/features/match/components/SoundToggle.tsx` 생성
- [ ] 아이콘: `Volume2` / `VolumeX` (lucide-react)
- [ ] `MatchStatusBar` 우측에 배치
...
| 사운드           | 트리거 시점                                                          |
| ---------------- | -------------------------------------------------------------------- |
| `submit`         | 내 `handleSubmit` 성공 직후                                          |
| `opponentSubmit` | `handleOpponentSubmitted` 콜백 내부                                  |
| `warning`        | 타이머가 `isCritical`로 **처음 진입할 때** (1회만)                   |
| `win`            | `handleMatchFinished`에서 `winnerId === userId`                      |
| `lose`           | `handleMatchFinished`에서 `winnerId !== userId && winnerId !== null` |

**주의사항**:

- `warning`은 매초 발생하면 안 되므로 `useRef` 플래그로 1회 가드
- 매치 시작 전(`PLAYER_READY` 이전)에는 재생 시도하지 않음 (브라우저 자동재생 정책)
...
- [ ] 효과음 파일 5종 다운로드 및 `public/sounds/` 배치
